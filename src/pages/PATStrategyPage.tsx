...
import { ArrowLeft, Brain, Lightbulb, AlertTriangle, Check, ChevronRight, Target } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';

const patCategories: Record<string, {
  title: string;
  color: string;
  bgColor: string;
  description: string;
  ...
  'keyholes': {
    ...
    color: '#14B8A6',
    bgColor: '#F0FDFA',
    description: 'Visualize a 3D object and determine which 2D aperture it can pass through.',
    questions: 15,
    targetTime: '~30 sec/q',
    difficulty: 'Medium',
    strategy: [
    ...
    tips: [
      ...
      'Use our 3D model viewer to rotate objects and understand from all angles',
      'Time yourself - 30 seconds per question is the sweet spot',
      'If stuck between two options, pick the more conservative fit',
    ],
    exampleWalkthrough: 'Look for the most restrictive feature (deepest groove or tallest ridge). Compare this feature against each aperture option. The correct aperture must accommodate ALL features of the object at ANY orientation.',
  },
  'tfe': {
    title: 'Top-Front-End',
    color: '#6366F1',
    bgColor: '#EEF2FF',
    description: 'Given two orthographic views (top, front, or end), determine the missing third view.',
    questions: 15,
    targetTime: '~45 sec/q',
    difficulty: 'Hard',
    strategy: [
    ...
    exampleWalkthrough: 'Align the two given views. For each feature (edge, hole, notch), trace how it would appear in the missing view using orthographic projection. Compare your mental image with the answer choices.',
  },
  'angle-ranking': {
    title: 'Angle Ranking',
    color: '#F59E0B',
    ...
    exampleWalkthrough: 'First, identify the obviously smallest and largest angles. Then compare the remaining two. For very close angles, mentally draw a circle around the vertex and compare arc lengths.',
  },
  'hole-punching': {
    title: 'Hole Punching',
    color: '#F43F5E',
    bgColor: '#FFF1F2',
    description: 'Mentally unfold a folded paper and determine where the hole(s) appear.',
    questions: 15,
    targetTime: '~40 sec/q',
    difficulty: 'Hard',
    strategy: [
    ...
    exampleWalkthrough: 'Start from the final folded state. For the last fold, determine where the hole would be on the other side of the fold line (mirror image). Repeat for each fold in reverse order. The final pattern shows all hole positions.',
  },
  'cube-counting': {
    title: 'Cube Counting',
    color: '#10B981',
    bgColor: '#ECFDF5',
    description: 'Count the number of cubes with a specific number of painted faces in a 3D stack.',
    questions: 15,
    targetTime: '~36 sec/q',
    difficulty: 'Medium',
    strategy: [
    ...
    tips: [
      ...
      'The "exploded view" in our 3D viewer helps you see all cubes',
      'Practice mental decomposition: break the structure into layers',
      '0-painted cubes = total cubes - surface cubes',
    ],
    exampleWalkthrough: 'Count total cubes first. Then identify cubes with 3 painted faces (corners), 2 painted faces (edges), 1 painted face (surface centers), and 0 painted faces (internal). Use the tally table to keep track.',
  },
  'pattern-folding': {
    title: 'Pattern Folding',
    color: '#8B5CF6',
    bgColor: '#F5F3FF',
    description: 'Determine which 3D form results from folding a 2D pattern/net.',
    questions: 15,
    targetTime: '~66 sec/q',
    ...
    tips: [
      ...
      'Use our fold animation to see the 2D→3D transition',
    ...
    exampleWalkthrough: 'Pick a reference face on the net. Identify which faces will be adjacent to it after folding. Check each option: does it have the correct faces adjacent? Are opposite faces actually opposite? Use elimination to find the only possible answer.',
  },
};

export default function PATStrategyPage() {
  const { category = 'keyholes' } = useParams<{ category: string }>();
  const data = patCategories[category] || patCategories['keyholes'];

  return (
    <main className="min-h-screen bg-[#F8FAFC]">
      <div className="pt-24 pb-16" style={{ background: `linear-gradient(135deg, ${data.color}22, ${data.color}11)` }}>
        <div className="section-container max-w-4xl mx-auto">
          <Link to="/" className="inline-flex items-center gap-2 text-[#475569] hover:text-[#0F172A] text-sm mb-6 transition-colors">
            <ArrowLeft className="w-4 h-4" />
            Back to Home
          </Link>
          <div className="flex items-center gap-3 mb-3">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: data.color }}>
              <Brain className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-[#0F172A]">{data.title} Strategy Guide</h1>
              <p className="text-sm text-[#475569]">PAT Section Guide</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="section-container max-w-4xl mx-auto -mt-8 pb-20">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { label: 'Questions', value: `${data.questions}`, color: data.color },
        ...
        <Card className="border-[#E2E8F0] mb-8">
          <CardContent className="p-6">
            <h2 className="text-lg font-bold text-[#0F172A] mb-2">What is {data.title}?</h2>
        ...
        <Card className="border-[#E2E8F0] mb-8">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-[#0F172A]">Step-by-Step Strategy</h2>
            </div>
            <ol className="space-y-3">
              {data.strategy.map((step, i) => (
                <li key={i} className="flex items-start gap-3">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 mt-0.5" style={{ backgroundColor: data.color }}>
                    {i + 1}
                  </div>
        ...
        <Card className="border-[#E2E8F0] mb-8">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-[#0F172A]">Common Pitfalls to Avoid</h2>
            </div>
            <ul className="space-y-2">
              {data.commonPitfalls.map((pitfall, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="text-[#EF4444] mt-0.5">&times;</span>
                  <span className="text-sm text-[#475569]">{pitfall}</span>
                </li>
              ))}
            </ul>
          </CardContent>
        ...
        <Card className="border-[#E2E8F0] mb-8">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-[#0F172A]">Pro Tips</h2>
            </div>
            <ul className="space-y-2">
              {data.tips.map((tip, i) => (
                <li key={i} className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-[#10B981] mt-0.5 shrink-0" />
                  <span className="text-sm text-[#475569]">{tip}</span>
                </li>
              ))}
            </ul>
          </CardContent>
        ...
        {/* Example Walkthrough */}
        <Card className="border-[#E2E8F0] mb-8" style={{ borderColor: data.color }}>
          <CardContent className="p-6">
            <h2 className="text-lg font-bold text-[#0F172A] mb-3">Example Walkthrough</h2>
            <p className="text-sm text-[#475569] leading-relaxed p-4 rounded-lg" style={{ backgroundColor: data.bgColor }}>
        ...
        <div className="p-6 rounded-xl bg-gradient-to-r from-[#0F172A] to-[#1E293B] text-white mb-8">
          <h3 className="text-lg font-semibold mb-2">Strategy Summary Card</h3>
          <div className="grid sm:grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-white/60 mb-1">Time per question</p>
              <p className="font-semibold">{data.targetTime}</p>
            </div>
            <div>
              <p className="text-white/60 mb-1">Target accuracy</p>
              <p className="font-semibold">80%+</p>
            </div>
            <div>
              <p className="text-white/60 mb-1">Key technique</p>
              <p className="font-semibold">{data.strategy[0]}</p>
            </div>
            <div>
              <p className="text-white/60 mb-1">Practice daily</p>
              <p className="font-semibold">20-30 questions</p>
            </div>
          </div>
        </div>
        ...
        <div className="text-center">
          <Button className="h-11 px-8 font-semibold" style={{ backgroundColor: data.color }}>
