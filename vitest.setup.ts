const DUMMY_DATABASE_URL = "postgresql://localhost:5432/test";
const testDatabaseUrl = process.env.TEST_DATABASE_URL?.trim();

// Database tests are opt-in. Never inherit DATABASE_URL or load the normal
// application .env file, because several integration tests delete shared rows.
if (testDatabaseUrl) {
  process.env.DATABASE_URL = testDatabaseUrl;
  process.env.TEST_DATABASE_AVAILABLE = "true";
} else {
  console.warn(
    "[vitest] TEST_DATABASE_URL not set — DB-dependent tests will be skipped."
  );
  process.env.DATABASE_URL = DUMMY_DATABASE_URL;
  process.env.TEST_DATABASE_AVAILABLE = "false";
}
